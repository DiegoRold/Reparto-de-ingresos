...
def procesar_reparto(df_datos):
    ...
    for index, row in df_datos.iterrows():                                          # 'iterrows()' nos da el índice de la fila y la fila misma (como un objeto).
        cod_matricula = row['cod_matricula']
        id_dim_producto = row['id_dim_producto']
        importe_matricula = row['importe_matricula']
        fec_matricula = pd.to_datetime(row['fec_matricula'], errors='coerce')       # errors='coerce' convierte una FechaNoValida en NaT
        ...
        if modalidad == 'ONLINE':
            ...
            if pd.notna(fecha_inicio_reconocimiento) and pd.notna(fecha_fin_reconocimiento):            # 'pd.notna()' comprueba si un valor no es nulo o NaT (Not a Time).
            ...
            # Regla 3: Si tampoco hay fechas de producto, usar fecha de matrícula + meses de duración.
                ...
                fecha_inicio_reparto = fec_matricula
                fecha_fin_reparto = fec_matricula + timedelta(days=int(meses_duracion * 30))                # timedelta permite sumar días, semanas, etc., a una fecha.
